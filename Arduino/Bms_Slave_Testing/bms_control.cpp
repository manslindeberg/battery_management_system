...
/*Sends to serial in order to query to InfluxDB using Node-Red*/
void send_volt_db(){
uint8_t incr = 0;
Serial.print("\r\n voltage, ");
 for (int current_ic = 0 ; current_ic < TOTAL_IC; current_ic++)
  {    
      ...
      {
        String cell_nam = "C" + String(incr) + "=";
        //uint16_t cell_val = BMS_IC[current_ic].cells.c_codes[i]*0.0001;
        float cell_val = random(3000,4000);
        //doc[cell_nam] = cell_val;
        Serial.print(cell_nam);
        Serial.print(cell_val*0.001,DEC);

        //if(!(current_ic == TOTAL_IC - 1 & i == BMS_IC[current_ic].ic_reg.cell_channels - 1)){
          Serial.print(", ");
        //}
...
/*Checks that cell voltages are within limit, note that the limit are only for testing right now.*/
bool voltage_limit_check()
{
  total_voltage = 0;
  for (int current_ic = 0 ; current_ic < TOTAL_IC; current_ic++)
    {
      for (int i=0; i< BMS_IC[current_ic].ic_reg.cell_channels; i++)
        {      
          if(BMS_IC[current_ic].cells.c_codes[i] > 67000){
            total_voltage = 0;
            return false;
          }
          total_voltage += BMS_IC[current_ic].cells.c_codes[i];
        }
    }
    return true;
}

bool temp_limit_check(){
   for (int current_ic = 0 ; current_ic < TOTAL_IC; current_ic++)
    {
      for (int i=0; i< BMS_IC[current_ic].ic_reg.aux_channels; i++)
        {      
          if(BMS_IC[current_ic].aux.a_codes[9] > 42000){
            return false;
          }
        }
    }
...
/*Checking Current from Allegro Current Sensor MAX: 150 A */
bool current_limit_check(){
  double current = 100*analogRead(16)*5/1024 - 249.5;
  (current < 150) ? true:false;
  
}

/*Initalizing slave units by writing into configuration registers & setting error codes/limits.**/
bool slave_init(){
...
/* Callback function when detection of upper or lower limit 
thresholds in voltage or temperature. If we are measuring 
during charging, we stop the CAN transmission to charger & go to passive state, 
otherwise going into shutdown state. */
